...
- **Theme.** `sub_406AF0` ends a level's setup with `rand() % 3` over `Theme1`, `Theme2`
  ...
  the port decision on the theme under "What a level plays" below). `Menu2` plays on the
  coworker-names screen and the highscore board. See docs/shell-reference.md.
