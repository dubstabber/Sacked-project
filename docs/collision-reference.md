...
`scenes/shared/grid_collision.gd` implements these neighborhood, line-intersection, and coordinate-clamping rules in logical coordinates for the original eight movement directions. It splits requested motion into steps of at most 0.25 tiles per axis so large physics ticks cannot jump past blockers. This subdivision is a port robustness measure, not a recovered original constant. Each step still uses the recovered 0.35/0.36 clearances and checks crossed cells. The helper also uses `floor(position + 0.5)` to support authored maps at negative coordinates; the original positive-coordinate maps use truncating integer conversion after adding 0.5.

For other movement angles, including small numerical deviations after coordinate conversion, the helper sweeps the same square footprint against blocking cells and slides along logical axes at contact. The same fallback handles an original corner correction that would leave the footprint overlapping another blocking cell. These are deliberate robustness extensions: the original side-line tests assume eight-direction input and can miss a boundary when an arbitrary shallow approach places the center-line intersection beyond the side's endpoints, and their sequential corner corrections can push into an adjacent obstacle. Both paths retain the original footprint and contact clearances.
