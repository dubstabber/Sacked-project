...
func _init() -> void:
	...
	var first_angle: int = cursor.get_movement_arrow_angle_from_phase(1.10)
	var second_angle: int = cursor.get_movement_arrow_angle_from_phase(1.25)
	if first_angle != second_angle:
		_fail("nearby phases should keep the same active cursor texture")
